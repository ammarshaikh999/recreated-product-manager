...
def update_product(site_url, product_id, new_description):
    r = wc_request(
...
def scan_one(site_url, product_url):
    ...
    api_matches = phrase_matches_in_html(description)
    ...
    if api_matches:
        return {
            ...
            "ID": product.get("id", ""),
            "Status": "✅ Found",
            "Detected Text": api_matches[0],
            "Details": "Found in WooCommerce product content."
        }

    # Critical fallback: the visible page may be built by Elementor/theme/custom fields.
    live = fetch_live_product_html(product_url)
    ...
    return {
        ...
        "Status": "❌ Not Found",
        "Detected Text": "",
        "Details": "Phrase not found in WooCommerce content or live product page."
    }
...
def process_one(site_url, product_url, dry_run=True):
    ...
    if html_contains_phrase(description):
        return {
            "URL": product_url, "Product": product.get("name",""),
            "ID": product.get("id",""), "Status": "⏭️ Already Present",
            "Details": "The phrase already exists; no duplicate added."
        }

    new_description, state = add_phrase_to_specifications(description)

    if state == "specifications_not_found":
        return {
            "URL": product_url, "Product": product.get("name",""),
            "ID": product.get("id",""), "Status": "⚠️ Specifications Not Found",
            "Details": "Could not locate 'Product Specifications' in the product description."
        }
    ...
    if dry_run:
        return {
            ...
            "Details": "Would add the phrase. No changes were sent to WordPress."
        }

    ok, msg = update_product(site_url, product["id"], new_description)
    return {
        "URL": product_url, "Product": product.get("name",""),
        "ID": product.get("id",""),
        "Status": "✅ Updated" if ok else "❌ Update Failed",
        "Details": "Phrase added successfully." if ok else str(msg)
    }

# ---------- UI ----------
st.title("🔎 Recreated Product Manager")
st.caption("Scan WooCommerce products and add “This is a Recreated Product” inside Product Specifications. Scanner checks WooCommerce content AND the live product page.")
...
st.divider()
st.caption("Important: this version targets the WooCommerce REST API and the product description HTML. Test with Preview / Dry Run before making live updates.")
